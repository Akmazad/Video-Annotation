...
$(document).ready(function() {
	...
	//TODO: A lot of this should be done in the backend in the future
/*	
	//If there's no theming content, remove the theming tab
	if($("#docs-theming .noarticletext").length) {
		$("#docs-theming").remove();
		$("#widget-docs ul:eq(0) a[href='#docs-theming']").remove();
	}
	
	//Hide not needed headings
	$('#widget-docs div.param-details h4').each(function() {
		if(!$(this).next().length) return $(this).hide();
		if($(this).next().is('table') && !$(this).next().find('td').length) {
			return $(this).hide() && $(this).next().hide();
		}
	});
	
	//Prepare the param types
	...
	$('#widget-docs p.param-type').each(function() {
		
		var html = $('span', this).remove().html();
		var words = $.map(html.split(','), function(n) { return paramDescriptions[$.trim(n)] ? '<span>'+$.trim(n)+'</span>' : $.trim(n); });
		$(this).html(words.join(', '));
		
		$('span', this).click(function() {
			if(paramDescriptions[this.innerHTML]) $('<div>'+paramDescriptions[this.innerHTML]+'</div>').dialog({ open: function() { $(this).parent().attr('id', 'demo-dialog'); }, modal: true, title: this.innerHTML, overlay: { background: '#000', opacity: 0.5 } });
		});
	...
	$(".param-header h3").click(function() {
		$(this).parent().parent().toggleClass("param-open").end().next().toggle();
	});
...
function gotoHash() {
	...
	$("#widget-docs").tabs('select', hash[0] == 'methods'? 2 : 1);
	var h3 = $("#widget-docs span:contains("+go+")").parent();
